...
|  Viratempo Alagoinhas (ex-Avaz, ex-Radar Politico)            |
|                                                               |
|  Pipeline:                                                    |
...
def _registrar_coleta(platform, data_type, items_count, status="ok", source_id=None):
    ...
    Monitor de Coleta do painel Viratempo. Best-effort: nunca derruba o pipeline.
...
def verificar_alertas(posts_analisados):
    ...
    mensagem  = f"*🚨 Viratempo · Alerta Automático ({data_hoje})*\n\n"
    mensagem += "\n".join(alertas)
...
def verificar_alerta_subtema(dry_run: bool = False):
    ...
    data_hoje = datetime.now().strftime("%d/%m/%Y %H:%M")
    mensagem = (f"*📈 Viratempo · Assuntos em alta ({data_hoje})*\n\n"
                + "\n".join(linhas)
...
def formatar_mensagem_alerta(post):
    ...
    linhas = [
        f"{emoji} *ALERTA · Viratempo Alagoinhas*",
        "",
...
def disparar_alertas(posts_analisados):
    ...
    rotulo_posts = "post em atenção" if len(disparados) == 1 else "posts em atenção"
    mensagem = f"🚨 *ALERTA AGORA · Viratempo ({data_hora})*\n{len(disparados)} {rotulo_posts}\n\n"
    mensagem += "\n\n──────────\n\n".join(partes)
...
def enviar_update_coments(post, motivo_update):
    ...
    linhas = [
        "🔔 *ATUALIZAÇÃO · Viratempo Alagoinhas*",
        "",
        f"*@{post.get('autor','')}* ({post.get('categoria','')})  ·  {post.get('data_post','')}",
...
# ==============================================================
# MODULO BOLETIM - BOLETIM CLIMATICO (painel Viratempo)
# ==============================================================
...
def main():
    ...
    _safe("boletim_climatico", gravar_boletim_climatico, posts_analisados)         # boletim climatico (painel Viratempo)
