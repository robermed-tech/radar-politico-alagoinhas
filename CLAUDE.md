...
- Ferramentas (custo zero: só leem texto já gravado):
  - `python agora.py --teste-localidade` roda uma bateria de casos com resposta esperada (`_CASOS_LOCALIDADE`), incluindo o do CTA e os três falsos positivos. **A amostra do banco que ele imprime depois alimenta o texto inteiro, não o valor extraído — a taxa de resolução ali é otimista e não vale como métrica de produção.**
  - `python agora.py --reparar-localidade [--dry-run]` desfaz atribuições vindas de nome composto. Sempre rodar o `--dry-run` e **ler os textos** antes de gravar.
