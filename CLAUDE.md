...
- **O critério mora numa constante só: `agora.py::CRITERIO_TOM_PUBLICACAO`**, interpolada em `PROMPT_TRIAGEM`, `PROMPT_SISTEMA` e `PROMPT_TOM`. Foi feito assim de propósito, por causa do que a auditoria de 26/07 encontrou: critério copiado em vários prompts diverge, e um deles fica um mês classificando pelo critério velho. Mexeu na constante, mexeu nos três.
...
- **`max_tokens` da triagem é 260, não 180.** O JSON ganhou dois campos; no teto antigo a resposta era cortada, o parse falhava e a base inteira caía nos defaults em silêncio (score_risco, urgência e tema zerados). O harness `--teste-triagem` usa o mesmo teto de propósito.
