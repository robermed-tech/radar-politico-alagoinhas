...
function PostCard({ p }: { p: Post }) {
  const texto = p.resumo || p.queixa_dominante || p.elogio_dominante || "";
  return (
    <div className="rounded-xl border border-line bg-bg-1 p-4">
      <div className="mb-2 flex items-center gap-2">
        ...
        <span className="ml-auto shrink-0 text-[10px] text-txt-3">{tempoRelativo(p.data_post)}</span>
      </div>
      {texto && (
        <p className="mb-2.5 line-clamp-2 text-sm text-txt-2">{texto}</p>
      )}
      <PostChips
  ...
  return (
    <div className="space-y-4 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-extrabold">Feed de Menções</h1>
          <p className="text-sm text-txt-2">{posts.length} publicações</p>
        </div>
